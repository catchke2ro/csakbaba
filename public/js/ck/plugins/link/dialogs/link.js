...
CKEDITOR.dialog.add( 'link', function( editor ) {
	...
	return {
		...
		minWidth: 600,
		minHeight: 400,
		onSelectPage: function(ev){
			if(ev.data.page=='extAtt'){
				this.dialog.getContentElement('extAtt', 'uploader').renderExt();
			}
		},
		contents: [
			{
			...
			{
				id: 'extAtt',
				label: linkLang.attachment,
				title: linkLang.attachment,
				elements: [
					{
						type: 'html',
						id: 'uploader',
						html: '',
						renderExt: function(){
							var created=Ext.ComponentQuery.query('panel[itemId=ckAttUploader]');
							if(created.length==0 || created.pop().editor!=CKEDITOR.dialog.getCurrent().getParentEditor().id){
								this.getElement().setHtml('<div class="extAttHandler" id="extAttHandler'+CKEDITOR.dialog.getCurrent().getParentEditor().id+'"></div>');
								Ext.create('SRG.upload', {
									fileType: 'all',
									itemId: 'ckAttUploader',
									renderTo: 'extAttHandler'+CKEDITOR.dialog.getCurrent().getParentEditor().id,
									width: 600,
									height:400,
									editor: CKEDITOR.dialog.getCurrent().getParentEditor().id,
									selectedOperation: function(view, record){
										var dialog=CKEDITOR.dialog.getCurrent();
										dialog.getContentElement('info', 'url').setValue(record.get('url'));
		...
		onHide: function() {
			//if(typeof Ext.getCmp('ckUploader')!='undefined'){Ext.destroy(Ext.getCmp('ckUploader'));}
		},
		onShow: function() {
		...
		onOk: function() {
			//if(typeof Ext.getCmp('ckUploader')!='undefined'){console.log(Ext.getCmp('ckUploader')); Ext.destroy(Ext.getCmp('ckUploader'));}
			var attributes = {},
				removeAttributes = [],
				data = {},
				me = this,
		...
		onLoad: function() {
			...
			this.on('selectPage', function(e){
				this.definition.onSelectPage(e);
			});
