	...
	var imageDialog = function( editor, dialogType ) {
			...
			return {
				...
				onSelectPage: function(ev){
					if(ev.data.page=='ext'){
						this.dialog.getContentElement('ext', 'uploader').renderExt();
					}
				},
				contents: [
					{
						id: 'ext',
						label: editor.lang.image.selectImg,
						elements: [
							{
								type: 'html',
								id: 'uploader',
								html: '',
								renderExt: function(){
									var created=Ext.ComponentQuery.query('panel[itemId=ckImgUploader]');
									if(created.length==0 || created.pop().editor!=CKEDITOR.dialog.getCurrent().getParentEditor().id){
										this.getElement().setHtml('<div class="extImgHandler" id="extImgHandler'+CKEDITOR.dialog.getCurrent().getParentEditor().id+'"></div>');
										Ext.create('CB.upload', {
											fileType: 'image',
											itemId: 'ckImgUploader',
											renderTo: 'extImgHandler'+CKEDITOR.dialog.getCurrent().getParentEditor().id,
											width: 600,
											height:400,
											editor: CKEDITOR.dialog.getCurrent().getParentEditor().id,
											selectedOperation: function(view, record){
												var dialog=CKEDITOR.dialog.getCurrent();
												dialog.getContentElement('info', 'txtUrl').setValue(record.get('url'));
